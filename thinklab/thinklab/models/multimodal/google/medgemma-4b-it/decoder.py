"""
Gemma 3 decoder for MedGemma 4B.
Key: QK-norm, sliding window + global attention, no softcap.
"""
import torch
...
class Gemma3ForCausalLM(nn.Module):
    """Gemma 3 causal LM. Weight keys match HF prefix `language_model.`."""
    ...
    def forward(self, input_ids=None, inputs_embeds=None, mask=None,
        ...
        if self.model.final_logit_softcap is not None:
            logits = torch.tanh(logits / self.model.final_logit_softcap) * self.model.final_logit_softcap
        return logits, attn
