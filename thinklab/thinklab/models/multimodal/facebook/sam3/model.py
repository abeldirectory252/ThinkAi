...
class CLIPAttention(nn.Module):
    """Keys: k_proj, v_proj, q_proj, out_proj (matching HF CLIP)."""
...
class CLIPEncoderLayer(nn.Module):
    """Keys: self_attn, layer_norm1, mlp, layer_norm2."""
...
class CLIPTextModel(nn.Module):
    """Keys: embeddings, encoder, final_layer_norm."""
...
class Sam3Model(BaseModel):
    ...
    Top-level submodules match HF Sam3Model exactly:
      vision_encoder, text_encoder, text_projection,
      geometry_encoder, detr_encoder, detr_decoder,
      mask_decoder, dot_product_scoring
    """
    model_type = "sam3"
    ...
    def __init__(self, vision_cfg=None, text_cfg=None, decoder_cfg=None,
        ...
        self.vision_encoder = Sam3VisionModel(
            ...
            pretrain_image_size=vc.get("pretrain_image_size", 1008),
            fpn_hidden_size=decoder_hidden,
            window_size=vc.get("window_size", 8),
            global_attn_indexes=vc.get("global_attn_indexes", []),
        ...
        # ── Text projection (CLIP hidden → decoder hidden) ──
        self.text_projection = nn.Linear(tc.get("hidden_size", 1024),
                                          decoder_hidden)
    ...
    def load_weights(self, path: Path, debug: bool = False):
        """Load weights from safetensors, stripping 'detector_model.' prefix."""
        ...
        # Skip tracker keys (SAM3 video tracker, not needed for image segmentation)
        cleaned = {k: v for k, v in cleaned.items()
                    if not k.startswith("tracker_model.") and not k.startswith("tracker_neck.")}
        ...
        if not missing and not unexpected:
            logger.info("✅ All %d weights loaded from %s", len(cleaned), path)
