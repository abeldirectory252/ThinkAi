...
def apply_rotary_pos_emb_2d(q, k, cos, sin):
    """q, k: [B, num_heads, seq, head_dim]. cos, sin: [seq_max, head_dim]."""
    seq_len = q.shape[2]
    cos = cos[:seq_len]
    sin = sin[:seq_len]
    # HF computes in float32 then casts back
    q_embed = q.float()
...
class Sam3ViTRotaryEmbedding(nn.Module):
    def __init__(self, hidden_size, num_heads, end_x, end_y,
        ...
        if dim % 4 != 0:
            raise ValueError("Dim must be divisible by 4 for axial RoPE")
...
class Sam3VisionModel(nn.Module):
    def __init__(self, hidden_size=1024, num_heads=16, intermediate_size=4096,
        ...
        self.backbone = Sam3ViTModel(
            ...
            window_size=window_size, global_attn_indexes=global_attn_indexes,
            layer_norm_eps=layer_norm_eps, dropout=dropout, rope_theta=rope_theta,
        )
