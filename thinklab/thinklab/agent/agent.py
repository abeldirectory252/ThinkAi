...
You can include reasoning text before the tool call.
After each tool call, you will see the result and can decide the next step.
When finished, use the "finish" tool with your final answer.

Rules:
- Think step by step
- Execute code to verify your reasoning
- If code fails, read the error and fix it
- Use the vision model for image analysis tasks
- Always provide a final answer via the "finish" tool
"""


class ThinkLabAgent:
    """ReAct agent that uses an LLM + sandbox to solve tasks."""
    ...
    def _tool_analyze(self, image_path: str, prompt: str) -> dict:
        ...
        return {"text": result.model_output}

    def _tool_explain(self, image_path: str, prompt: str,
                      mode: str = "both") -> dict:
        result = self.tm.inference(
            image_path, prompt, max_tokens=128,
            explainability={"enabled": True, "mode": mode, "per_class": True},
        )
        out = {"text": result.model_output}
        if result.explain:
            out["mode"] = result.explain.mode
            out["total_heatmaps"] = result.explain.total_heatmaps
            out["mean_overlap"] = result.explain.mean_overlap
            if result.explain.grad_cam_labels:
                out["grad_cam_labels"] = result.explain.grad_cam_labels
            if result.explain.lime_weights:
                top = sorted(result.explain.lime_weights.items(), key=lambda x: -x[1])[:5]
                out["top_lime_segments"] = top
        return out
